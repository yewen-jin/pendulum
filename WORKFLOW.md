...
| 2026-04-15 | Phone files consolidated on collaborator pair | Deleted `pendulum.mmp`/`pendulum.pd`; `pendulum1.mmp`/`pendulum1.pd` is the canonical pair (performer uses `pendulum1` on phone). Pointed `.mmp` at its own `.pd` and added `/runToggle` → `/phone/runToggle` routing |
